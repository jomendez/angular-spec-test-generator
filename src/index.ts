...
export class App {
  ...
  async app() {
    try {
      ...
      const handlerFile = this.getSpecType(this.getFileTree(sourceDir), sourceTypes);
...
`;
  ...
  private getFileTree(sourceUrl: string) {
    ...
    files.forEach((file) => {
      ...
      if (fs.lstatSync(url).isDirectory()) {
        fileNAmeArray.push(...this.getFileTree(url));
      }
  ...
  private getSpecType(array: Array<string>, sourceTypes: string[] = []) {
    const sourceArray = [];
    ...
    for (const url of array) {
      ...
      if (file.ext === '.ts' && !url.includes('.spec')) {
        ...
        const type = sourceTypes.find(sourceType => url.includes(sourceType));
